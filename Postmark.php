...
class Mail_Emailmanager
{
	...
	const TESTING_API_KEY = 'EMAILMANAGER_API_TEST';
	...
	*/
	...
	{
		if (class_exists('Mail_Emailmanager_Adapter')) {
			require_once('Adapter_Interface.php');
			
			$reflection = new ReflectionClass('Mail_Emailmanager_Adapter');
			
			if (!$reflection->implementsInterface('Mail_Emailmanager_Adapter_Interface')) {
				trigger_error('Mail_Emailmanager_Adapter must implement interface Mail_Emailmanager_Adapter_Interface', E_USER_ERROR);
			}
			
			$this->_apiKey = Mail_Emailmanager_Adapter::getApiKey();
			
			Mail_Emailmanager_Adapter::setupDefaults($this);
			
		} else {
			if (!defined('EMAILMANAGERAPP_API_KEY')) {
				trigger_error('Emailmanager API key is not set', E_USER_ERROR);
			}
			
			$this->_apiKey = EMAILMANAGERAPP_API_KEY;
			
			if (defined('EMAILMANAGERAPP_MAIL_FROM_ADDRESS')) {
				$this->from(
					EMAILMANAGERAPP_MAIL_FROM_ADDRESS,
					defined('EMAILMANAGERAPP_MAIL_FROM_NAME') ? EMAILMANAGERAPP_MAIL_FROM_NAME : null
				);
			}
		}
	...
	* must first be added in the Emailmanager admin interface
	* 
	...
	* 
	* @throws Exception If HTTP code 422, Exception with API error code and Emailmanager message, otherwise HTTP code.
	...
	*/
	public function send()
	{
		...
		$headers = array(
			...
			'X-Emailmanager-Server-Token: ' . $this->_apiKey
		);
		...
		if (!$this->_isTwoHundred($httpCode)) {
			if ($httpCode == 422) {
				...
				throw new Exception("Error while mailing. Emailmanager returned HTTP code {$httpCode} with message \"{$return}\"", $httpCode);
			}
		}
	...
	*/
	...
	{
		if (class_exists('Mail_Emailmanager_Adapter')) {
			Mail_Emailmanager_Adapter::log($logData);
		}
	}
